...
function buildCookiesData(cookies: puppeteer.Protocol.Network.Cookie[]): string {
  return cookies
    .sort((cookieA: puppeteer.Protocol.Network.Cookie, cookieB: puppeteer.Protocol.Network.Cookie) : number => {
      return cookieA.domain > cookieB.domain ? 1 : -1;
    })
  .reduce((data: string, cookie: puppeteer.Protocol.Network.Cookie) : string => {
    const newRow = [
